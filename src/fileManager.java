...
public class fileManager {
	...
	private PhysicalMemory wFizMemory;
	
	public fileManager(PhysicalMemory wFizMemory) {
	...
	public File getForCut() {
		return forMove;
	}

	public void setForCut(File forMove) {
	...
	public void copyFiles(File newFile) {
		for (File file : newFile.getChildren()) {
	...
	public void startDelForlder() {
		...
		delForder(selected.getChildren());
	}

	public void delForder(ArrayList<File> files) {
		for (File file : files) {
			if(file.getFolder()) {
				delForder(file.getChildren());
			}
	...
	public boolean cut() {
		if(Objects.isNull(forMove)) {
			...
			if(selected.getFolder()) {
				forMove.getParrent().getChildren().remove(forMove);
				forMove.setParrent(selected);
				forMove.getParrent().getChildren().add(forMove);
				forMove = null;
				return true;
			} else {
				return false;
	...
	public boolean delete() {
		if(selected == rootFile) {
			...
			selected.getParrent().getChildren().remove(selected);
			if (selected.getFolder()) {
				startDelForlder();;
			} else {
				wFizMemory.clearMemory(selected);
